...
// import { useState, useEffect } from "react";
// import { Link, useLocation } from "wouter";
// import { Gift, Search, Home, Compass, Sparkles, MessageSquare, LogOut, LogIn, ShoppingCart, Package, User } from "lucide-react";
// import { cn } from "@/lib/utils";
// import { getToken, clearToken } from "@/lib/auth";
// import { getCart } from "@/lib/cart";
...
//                   location === "/orders" ? "text-violet-600 bg-violet-100" : "text-gray-500 hover:text-gray-800 hover:bg-violet-50"
//                 )}>
//                   <Package className="w-4 h-4" />
//                   Orders
//                 </div>
//               </Link>
//               <Link href="/cart">
//                 <div className="relative p-2 rounded-lg text-gray-500 hover:text-violet-600 hover:bg-violet-50 transition-colors cursor-pointer">
//                   <ShoppingCart className="w-5 h-5" />
//                   {cartCount > 0 && (
...
//             <div className="flex items-center gap-2">
//               <Link href="/profile">
//                 <div className="hidden sm:flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-sm font-medium text-gray-500 hover:text-violet-600 hover:bg-violet-50 transition-colors cursor-pointer">
//                   <div className="w-6 h-6 rounded-full bg-violet-100 flex items-center justify-center text-xs font-bold text-violet-700">
//                     {user.name?.[0]?.toUpperCase() || "U"}
//                   </div>
//                   <span className="hidden lg:block">{user.name?.split(" ")[0]}</span>
...
//                 className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-gray-500 hover:text-gray-800 hover:bg-violet-50 transition-colors"
//               >
...
//           ) : (
//             <Link href="/login">
//               <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold bg-violet-600 text-white hover:opacity-90 transition-opacity">
//                 <LogIn className="w-4 h-4" />
//                 <span>Login</span>
//               </div>
//             </Link>
...
//       {/* Mobile bottom nav */}
//       <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-violet-100 z-50">
//         <div className="grid grid-cols-5 h-14">
//           {mobileLinks.map(({ href, label, icon: Icon }) => (
//             <Link key={href} href={href}>
//               <div className={cn(
//                 "flex flex-col items-center justify-center h-full gap-0.5 text-[10px] font-medium transition-colors",
//                 location === href ? "text-violet-600" : "text-gray-400"
//               )}>
